...
def get_reasoning_format(model: dict) -> str | None:
    ...
    Returns:
        ...
        'reasoning_content': OpenAI-compatible providers that support reasoning_content
                             as a top-level field (llama.cpp, MiMo, DeepSeek, etc.).
        None: skip reasoning (safe default for strict providers).
    """
    provider = model.get('provider', '')
    if provider == 'ollama':
        return 'think_tags'
    # All OpenAI-compatible providers: use reasoning_content field.
    # Providers that produce reasoning content (MiMo, DeepSeek, etc.)
    # require it to be passed back on follow-up calls (e.g. after tool use).
    # Providers that don't support it will simply ignore the extra field.
